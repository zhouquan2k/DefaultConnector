...
import component.cti.protocol.ReconnectCallRequest;
import component.util.Future;
import component.util.SyncMode;
import component.util.Util;

public class RequestReconnectCallPDU extends RequestPDU {
	...
	public void invokeAPI(UDPHandler udpHandler) throws Throwable {
		...
		Util.trace(this,
				"ReconnectCall request ... deviceId=" + this.getDeviceID()
						+ ",activeCallId=" + this.getActiveUCID()
						+ ",heldCallId=" + this.getHeldUCID());
		...
		if (active_callInfo != null) {
			...
			Util.trace(
					this,
					"ReconnectCall request convert active ucid="
							+ this.getActiveUCID() + ", to callID="
							+ this.getActiveCallID());
		} else {
			Util.trace(
					this,
					"ReconnectCall request callInfo is null. "
							+ this.getActiveUCID());
		...
		// Query heldCallInfo callID by heldUCID;
		CallInfo held_callInfo = udpHandler
				.getCallInfoManager()
				.getActiveCallInfoByUcid(this.getDeviceID(), this.getHeldUCID());
		if (held_callInfo != null) {
			this.setHeldCallID(held_callInfo.getCallID());
			Util.trace(
					this,
					"ReconnectCall request convert held ucid="
		...
		if (this.getDeviceID() != null) {
			...
			request.method = "reconnectCall";
			ReconnectCallRequest reconnectCall = new ReconnectCallRequest(
					this.deviceID, this.heldCallID, this.activeCallID);
			reconnectCall.setSyncMode(SyncMode.Async);
			request.params = new ReconnectCallRequest[] { reconnectCall };
			request.object = "cti";
			...
			if (!(result instanceof Future)) {
				if (result instanceof Throwable) {
					...
					ResponseReconnectCallPDU response = (ResponseReconnectCallPDU) UDPUtil
			...
			Util.trace(this, "ReconnectCall failure for deviceID is null.");
